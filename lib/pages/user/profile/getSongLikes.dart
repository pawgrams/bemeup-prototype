// Datei: pages/user/profile/getSonglikes.dart
...
class GetSonglikes extends StatefulWidget {
  final String visitorUserId;
  final String profileUserId;
  const GetSonglikes({super.key, required this.profileUserId, required this.visitorUserId});

  @override
  State<GetSonglikes> createState() => _GetSonglikesState();
}

class _GetSonglikesState extends State<GetSonglikes> {
  ...
  Future<void> _loadSongs() async {
    final userLikesSnap = await FirebaseDatabase.instance
        .ref('userlikes/${widget.profileUserId}')
        .get();
    if (!userLikesSnap.exists || userLikesSnap.value == null) {
      if (!mounted) return;
      setState(() => _ready = true);
      return;
    }

    final Map userLikes = Map<String, dynamic>.from(userLikesSnap.value as Map);
    final songIds = userLikes.keys.toList();
  ...
  Future<void> _prepareCovers() async {
    ...
    for (int i = 0; i < songs.length && i < songListStyle['itemsToWaitFor']; i++) {
      final song = songs[i];
      final isSuggested = (song['uuid'] ?? '').toString().startsWith('_');
      final highlight = (song['highlight'] ?? 0) is int && (song['highlight'] ?? 0) > DateTime.now().millisecondsSinceEpoch;
      if (highlight) {
        if (isSuggested) {
          _coverMap[song['uuid'] ?? ''] = const AssetImage('assets/defaults/cover.png');
        } else {
    ...
    return ScrollConfiguration(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, idx) {
          ...
          final image = _coverMap[song['uuid'] ?? ''] ?? (isSuggested
              ? const AssetImage('assets/defaults/cover.png')
              : const AssetImage('assets/defaults/cover.png'));

          return SizedBox(height: totalItemHeight, child: Padding(
            ...
            child: Stack(
              ...
              children: [
                ...
                Positioned.fill(
                  child: ClipRRect(
                    borderRadius: BorderRadius.circular(songListStyle['containerRadius']),
                    child: HighlightFilter(backgroundImage: image),
                  ),
                ),
                Align(alignment: Alignment.center, child: Container(height: songListStyle['containerHeight'], child: content)),
              ],
            ),
          ));
        },
